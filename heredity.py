...
def inherit_prob(parent_name, one_gene, two_genes):
    """
    Calculate the probability of a parent passing a copy of the mutated gene to their child.

    Args:
    - parent_name (str): The name of the parent.
    - one_gene (set): Set of people having 1 copy of the gene.
    - two_genes (set): Set of people having two copies of the gene.

    Returns:
    float: The probability of the parent passing a copy of the mutated gene to their child.
    """
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    """
    Update the probability distributions in `probabilities` with a new joint probability `p`.

    Args:
    - probabilities (dict): Dictionary containing probability distributions for each person.
    - one_gene (set): Set of people having 1 copy of the gene.
    - two_genes (set): Set of people having two copies of the gene.
    - have_trait (set): Set of people having the trait.
    - p (float): The probability to be added to the distributions.
    """
...
def normalize(probabilities):
    """
    Normalize each probability distribution in `probabilities` such that the sum of probabilities in each distribution equals 1.
